    ...
    function OnlineServices {
        $ShortcutFiles = Get-ChildItem "C:\ProgramData\Microsoft\Windows\Start Menu\Programs" -File -Filter *.lnk
        $URLFiles = Get-ChildItem "C:\ProgramData\Microsoft\Windows\Start Menu\Programs" -File -Filter *.url
        $PartialTargetPathToCheck = "C:\Program Files (x86)\Online Services"
        ...
        foreach ($ShortcutFile in $ShortcutFiles) {
            ...
            $WshShell = $null  # Release the COM object
    
            if ($IconLocation -like "*$PartialTargetPathToCheck*" -or $ShortcutTarget -like "*$PartialTargetPathToCheck*" -or $Arguments -like "*http*") {
                Write-Host "Deleting .lnk file: $($ShortcutFile.Name)"
                Remove-Item -Path $ShortcutFile.FullName -Force
            }
        }
        foreach ($URLFiles in $URLFiles) {
        if ($URLFiles) {
            Write-Host "Deleting .url file: $($URLFiles.Name)"
            Remove-Item -Path $URLFiles.FullName -Force
            }
        }
        Write-Host "Deleting Online Services Folder"
    ...
    $RemoveStartBloat = New-Object system.Windows.Forms.Button
    $RemoveStartBloat.text = "Remove Start Bloat"
    $RemoveStartBloat.width = 140
    $RemoveStartBloat.height = 40
    $RemoveStartBloat.location = New-Object System.Drawing.Point(370, 125)
    $RemoveStartBloat.Font = 'Microsoft Sans Serif,10'
    ...
    $Form.controls.AddRange(@($Debloat, $PleaseWaitLabel, $ITDB, $ISFile, $InsatllGames, $LongSleep, $RemoveStartBloat, $Theme, $DisableDarkMode, $EnableDarkMode, $CustomizeBlacklists, $RemoveAllBloatware, $RemoveBlacklist, $Label1, $BasicSS, $UnBasicSS, $Label2, $Label3, $DisableTaskbarbloat, $StopEdgePDFTakeover, $DisableTelemetry, $RemoveRegkeys, $FixWhitelist, $RemoveBloatNoBlacklist))
    ...
    $RemoveAllBloatware.Add_Click( { 
            ...
            DebloatAll
            Write-Host "Removing Online Services."
            OnlineServices
    ...
    $RemoveStartBloat.Add_Click( { 
        $ErrorActionPreference = 'silentlycontinue'
        Write-Host "Removing Online Services"
        OnlineServices
        Write-Host "Removed Online Services."
    })
    ...
    $BasicSS.Add_Click( {
        if (Test-WinGetInstalled) {
            Write-Host "WinGet is not installed. Please Wait."
    } else {
        $PleaseWaitLabel.Visible = $true
        function Install-WinGetPackage {
            ...
            $processInfo.Arguments = "winget install `"$packageName`" -h --accept-package-agreements --accept-source-agreements"
        ...
        Install-WinGetPackage Adobe.Acrobat.Reader.64-bit
        ...
        Install-WinGetPackage Unchecky.Unchecky
        ...
        Write-Host "Installing VLC With WinGet"
    ...
    $ISFile.Add_Click( {
        ...
        if (Test-ChocolateyInstalled) {
            ...
            $aiscriptPath = "$PSScriptRoot\autoinstaller.ps1"
    
            # Run the child script directly
            & $aiScriptPath
